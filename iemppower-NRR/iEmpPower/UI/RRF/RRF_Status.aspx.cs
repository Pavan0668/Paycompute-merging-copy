...
{
    public partial class RRF_Status : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                Load_Request_details(2);   //2 my req
                Page_init();
        ...
        protected void txtsearch_rrf_TextChanged(object sender, EventArgs e)
        {
            ...
            RrfBl ObjBl = new RrfBl();
            List<RrfBo> objoList = new List<RrfBo>();
            objoList = ObjBl.Get_RRFreq_search(User.Identity.Name, 1, 1, Convert.ToInt32(DDL_search_RRF.SelectedValue), txtsearch_rrf.Text.Trim());
            if (objoList.Count > 0)
            {
                GRD_RRF.DataSource = objoList;
                GRD_RRF.DataBind();
